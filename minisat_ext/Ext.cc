...
void SamplingSolver::set_prob_backward(const vector<pair<int, int> >& sample_prob){
    // 根据一个变量即将取 0 和取 1 的次数进行采样
    Minisat::Solver* sv = (Minisat::Solver*)internal_solver; 

    // do context-aware sampling
    if (!sample_prob.empty()){
        for (int i = 0; i < sv->nVars(); ++i){
            int l0 = sample_prob[i].first, l1 = sample_prob[i].second;
            if (l0 <= 0 && l1 <= 0) l0 = l1 = 1;
            // here 0 represents true and 1 represents false
            int t = gen() % (l0 + l1);
            if (t < l0){
                sv->setPolarity(i, (Minisat::lbool((uint8_t)1)));
            } else {
