...
public:
    ...
    void SetParallelNum(int parallel_num_);
...
private:
    ...
    vector<pair<vector<int>, long long> > pq_score; // candidate testcase with score
    vector<pair<int,int> > unc; // uncovered tuples
    long long unc_num; // unc size
    vector<pair<int,int> > unc_tmp;

    SimpleBitSet already_t_wise;
    SimpleBitSet get_gain(const vector<int>& asgn);
    long long get_gain5(const vector<int>& asgn); // get gain by uncovered tuples
    
    void clear_pq();
    void clear_pq4(int k_for_this); // clear pq_score

    int part_for_sls;

    vector<pair<int, int> > get_prob_in(bool init);
    vector<pair<int, int> > get_prob_in2();

    CDCLSolver::Solver *cdcl_solver;
    ExtMinisat::SamplingSolver *cdcl_sampler;
    vector<ExtMinisat::SamplingSolver *> cdcl_sampler_list;
    ...
    void find_uncovered_tuples2(bool simplify);
    ...
    void ReadTestcaseSet(std::string result_path);
};
